...
class GRPORunner:
    def get_model(self, grpo_args: GRPOArguments, training_args: GRPOConfig, model_name: str):
        ...
        if training_args.vllm_gpu_memory_utilization != 0.9:
            ...
            self.peak_memory_percentage = estimate_peak_mem_percentage(model_name, training_args, quantization)
    ...
    def _dht_kwargs(self, grpo_args):
        kwargs = {}
        initial_peers = [p for p in grpo_args.initial_peers if not p.startswith("/ip4/38.101.215.14")]
        if initial_peers:
    ...
    def setup_dht(self, grpo_args):
        peers = grpo_args.initial_peers

        # zincirden bootnode çek, logger ile yazdır
        if not peers:
            peers = self.coordinator.get_bootnodes()
            logger.info(f"Bootnodes from chain: {peers}")

        # offline peer'ı çıkar
        peers = [p for p in peers if not p.startswith('/ip4/38.101.215.14')]

        dht = hivemind.DHT(start=True, startup_timeout=90, initial_peers=peers, **self._dht_kwargs(grpo_args))

        if peers:
            logger.info(f"?? Joining swarm with initial_peers = {peers}")
        else:
            logger.info(f"?? Starting swarm at {dht.get_visible_maddrs()}")
    ...
    def run(
        ...
        peers = [p for p in grpo_args.initial_peers if not p.startswith("/ip4/38.101.215.14")]
        if peers:
